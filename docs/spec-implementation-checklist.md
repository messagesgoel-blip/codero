...
| Session Lifecycle v1 | partial | `internal/session/checkpoint.go` (19 lifecycle checkpoints), `internal/state/session_archives.go` (session_archives table, archive writer, queries), `internal/state/migrations/000014_session_archives.up.sql` (migration), `internal/state/agent_sessions.go` (archive write in FinalizeAgentSession + ExpireAgentSession), `cmd/codero/session_end.go` (codero session end command), `cmd/codero/session_bootstrap.go` (bootstrap), `cmd/codero/session_finalize.go` (finalize), `internal/state/session_lifecycle_v1_certification_test.go` (15 tests), `internal/session/checkpoint_test.go` (4 tests), `cmd/codero/session_end_test.go` (1 test). | 11 of 18 certification-matrix criteria pass (§1.1, §2, §4.1, §4.2, SL-1–SL-7). 7 criteria fail: SL-9 (tmux session IS session), SL-10 (tmux-native heartbeat), SL-11 (tmux naming), SL-12 (wrapper handles integration), SL-13 (14-step wrapper), SL-14 (unclean exit reporting), SL-15 (session log capture). tmux integration and wrapper not implemented. Cannot advance to `implemented` until SL-9–SL-15 are built. |

## Next Branch Candidates

1. All core specs (Agent v3, Task Layer v2, Daemon v2, Repo-Context v1, Gate Config v1, Review Gate v1, Dashboard API v1) are now `certified`. Advance to `done` once dependency chain is fully certified.
2. Session Lifecycle v1 is `partial` — DB-side lifecycle certified (§1.1, §2, §4.1, §4.2, SL-1–SL-7); tmux integration and wrapper (SL-9–SL-15) remain unimplemented.
3. Return to the still-partial specs from the authoritative index (Execution Loop v1, LiteLLM Chat v1, Real-Time Views v1).
4. After all specs in the dependency chain reach `done`, update the spec index.
