...
| Repo-Context v1 | done | `internal/context/` (store, indexer, queries, types), `cmd/codero/context_cmd.go` (8 CLI commands), `.codero/context/graph.db` store. | Full repo-local code-intelligence subsystem: SQLite graph store, Go AST indexer (full+incremental), 8 `codero context` commands (index/status/find/grep/symbols/deps/rdeps/impact), advisory-only impact analysis with risk heuristic, 24 tests. |
