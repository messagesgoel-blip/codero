...
main() {
  ...
  response="$(curl -sS -X POST "$LITELLM_URL" \
    ...
    -d "{\"model\":\"$LITELLM_MODEL\",\"messages\":[{\"role\":\"system\",\"content\":\"You are a senior software engineer performing a strict local pre-commit review.\"},{\"role\":\"user\",\"content\":$(printf '%s' "$prompt" | jq -Rs .)}],\"temperature\":0.2}")"
